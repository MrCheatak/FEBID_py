...
class ETrajMap3d(object):
    ...
    def setParametrs(self, structure, segment_min_length=0.3, **mc_params):
        ...
        self.amplifying_factor = 10000  # artificially increases SE yield to preserve accuracy
        self.emission_fraction = mc_params[
            'emission_fraction']  # fraction of total lost energy spent on secondary electron emission
        # self.e = e # fitting parameter related to energy required to initiate a SE cascade, material specific, eV
    ...
    def traverse_cells(self, p0, pn, direction, t, step_t):
        ...
        while True:  # iterating until the end of the ray
            next_t, ind = self.__arr_min(
                t)  # minimal t-value corresponds to the box wall crossed; 2x faster than t.min() !!
    ...
    def follow_segment(self, points, dEs):
        ...
        t = np.abs(
            (delta + (step == self.cell_dim) * self.cell_dim + (delta == 0) * step) / direction)  # initial t-value
        max_traversed_cells = int(
            L.max() / self.cell_dim * 2) + 10  # maximum number of cells traversed by a segment in the trajectory;
        # this is essential to allocate enough memory for the traversal algorithm
        self.DE = np.zeros_like(self.grid)
        traversal.traverse_segment(self.DE, self.grid, self.cell_dim, p0, pn, direction, t, step_t, des,
                                   max_traversed_cells)
    ...
    def generate_se(self):
        ...
        self.se_coords_all = coords_all = np.int32(
            ne.evaluate('a/b', global_dict={'a': self.coords_all.T, 'b': self.cell_dim}))
        ...
        traversal.generate_flux(self.flux, self.surface.view(dtype=np.uint8), self.cell_dim, coords, pn, direction,
