...
def flux_matrix(matrix, y1:int, y2:int, x1:int, x2:int):
    ...
    dist = re_evaluate_from_cache(expressions["flux_matrix"])
    irradiated_area[index_y, index_x] = pe_flux(dist)


# @jit(nopython=True)
def define_irradiated_area(beam:Positionb, effective_radius_relative:int):
    """
...
def printing(loops=1):
    ...
    dwell_step = 5 # int(beam_d / 2/cell_dimension)
    ...
    for l in range(loops):  # loop repeats, currently beam travels in a zig-zack manner (array indexing)
        ...
        for beam.y in range(y_offset, y_limit, dwell_step):  # beam travel along Y-axis
            for beam.x in range(x_offset, x_limit, dwell_step):  # beam travel alon X-axis
                y_start, y_end, x_start, x_end = define_irradiated_area(beam, effective_radius_relative) # Determining the area around the beam that is effectively irradiated
                ...
                while True:
                    ...
                    surf, ghosts_index, refresh_flag = update_surface(deposit[irradiated_area_3D], substrate[:surface.max()+3, y_start-2:y_end+2, x_start-2:x_end+2], surface[irradiated_area_2D], surf, semi_surface, ghosts, ghosts_index, y_start, x_start) # updating surface on a selected area
                    if t % refresh_dt < 1E-6:
                        if refresh_flag == True:
                            temp = list(zip(*semi_surface))
                            semi_surface_index = (asarray(temp[0]), asarray(temp[1]), asarray(temp[2]))
                            refresh_flag = False
